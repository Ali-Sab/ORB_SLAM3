...
class ImageGrabber{
public:
	ImageGrabber(ORB_SLAM3::System &_SLAM, ImuGrabber *_pImuGb,const bool _bRect, const bool _bClahe,ros::Publisher &_pub_pts_and_pose,
		ros::Publisher &_pub_all_kf_and_pts) :
		SLAM(_SLAM),mpImuGb(_pImuGb),do_rectify(_bRect), mbClahe(_bClahe), pub_pts_and_pose(_pub_pts_and_pose),
...
int main(int argc, char **argv){
	...
	ORB_SLAM3::System SLAM(argv[1], argv[2], ORB_SLAM3::System::IMU_STEREO, show_viewer);	
	...
	if (read_from_topic) {
		ImuGrabber imugb;
		ImageGrabber igb(SLAM,&imugb,sbRect == "true",bEqual,pub_pts_and_pose, pub_all_kf_and_pts);
		...
		ros::Subscriber sub_img_left = nodeHandler.subscribe("/camera/left/image_raw", 100, &ImageGrabber::GrabImageLeft,&igb); //image from esp 32 
        ros::Subscriber sub_img_right = nodeHandler.subscribe("image_topic", 100, &ImageGrabber::GrabImageRight,&igb); // image from tello camera
...
void ImageGrabber::SyncWithImu()
{
  ...
  while(1)
  {
    ...
    if (!imgLeftBuf.empty()&&!imgRightBuf.empty()&&!mpImuGb->imuBuf.empty())
    {
      ...
      SLAM.TrackStereo(imLeft,imRight,tImLeft,vImuMeas);
...
bool parseParams(int argc, char **argv) {
	if (argc < 7){
		cerr << endl << "Usage: rosrun ORB_SLAM3 Monopub path_to_vocabulary path_to_settings path_to_sequence/camera_id/-1 <image_topic> [do_equalize] do_rectify" << endl;
		return 1;
	}
